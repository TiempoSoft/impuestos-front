...
import Cuentas from '../ModalEditarRubros'
...
export default {
  ...
  methods :{
   handleClick() {
      this.$store.state.c.CodigoCuentaRubroVenta = this.codigo;
      this.$store.state.c.DescripCuentaRubroVenta = this.currentRow;
      this.$modal.hide("CuentaNueva");
    },
    handleCurrentChange(val) {
      this.todosLosDatos = val;
      this.codigo = val.cuenta;
      this.currentRow = val.descrip;
      console.log(this.currentRow);
    },
  },
  created() {
    Axios.get("http://localhost:3000/maestro").then((result) => {
